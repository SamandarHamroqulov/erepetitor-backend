...
exports.CREATE_MONTH = async (req, res) => {
  try {
    ...
    const students = await prisma.student.findMany({
      ...
      select: { id: true, customMonthlyFee: true, paymentStartDate: true },
    });
    ...
    // Filter out students whose start date is strictly after this month
    const validStudents = students.filter((s) => {
      if (!s.paymentStartDate) return true;
      const startMonth = s.paymentStartDate.toISOString().slice(0, 7);
      return startMonth <= month;
    });

    if (!validStudents.length) {
      return res.status(400).json({ message: "Bu oy uchun faol o'quvchi topilmadi (boshlanish sanasi bo'yicha)" });
    }
    ...
    const result = await prisma.payment.createMany({
      data: validStudents.map((s) => ({
        studentId: s.id,
        month,
        amount: s.customMonthlyFee ?? group.monthlyPrice,
        status: "DUE",
      })),
      skipDuplicates: true,
    });
...
exports.CREATE_ONE = async (req, res) => {
  try {
    ...
    if (student.paymentStartDate) {
      const startMonth = student.paymentStartDate.toISOString().slice(0, 7);
      if (startMonth > month) {
        return res.status(400).json({ message: "O'quvchining to'lov boshlanish sanasi bu oydan keyin" });
      }
    }
    ...
    const payment = await prisma.payment.create({
      data: {
        ...
        month,
        amount: student.customMonthlyFee ?? student.group.monthlyPrice,
        status: "DUE",
      },
    });
...
exports.DEBTORS = (req, res) => listPayments(req, res, { in: ["DUE", "PARTIAL"] });
